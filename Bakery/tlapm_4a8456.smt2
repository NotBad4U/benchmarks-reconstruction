...
;;	                 /\ VARIABLE_num_[CONSTANT_i_] = 0
;;	                    => VARIABLE_pc_[CONSTANT_i_] \in {"p1", "p2", "p3", "p7"}
;;	              /\ /\ VARIABLE_flag_[CONSTANT_i_]
;;	                    => VARIABLE_pc_[CONSTANT_i_] \in {"p1", "p2", "p3", "p4"}
;;	                 /\ VARIABLE_pc_[CONSTANT_i_] \in {"p2", "p3"}
;;	                    => VARIABLE_flag_[CONSTANT_i_]
;;	              /\ VARIABLE_pc_[CONSTANT_i_] \in {"p5", "p6"}
;;	                 => (\A CONSTANT_j_
;;	                        \in (1..CONSTANT_N_ \ VARIABLE_unread_[CONSTANT_i_])
;;	                            \ {CONSTANT_i_} :
;;	                        /\ VARIABLE_num_[CONSTANT_i_] > 0
...
;;	       \E CONSTANT_repeat_ \in BOOLEAN :
;;	          \E CONSTANT_k_ \in Nat :
;;	             IF CONSTANT_repeat_
;;	               THEN /\ ?VARIABLE_num_#prime
;;	                       = [VARIABLE_num_ EXCEPT
;;	                            ![CONSTANT_self_] = CONSTANT_k_]
;;	                    /\ ?VARIABLE_pc_#prime
;;	                       = [VARIABLE_pc_ EXCEPT ![CONSTANT_self_] = "p3"]
;;	               ELSE /\ \E CONSTANT_i_
;;	                          \in {CONSTANT_j_ \in Nat :
;;	                                 CONSTANT_j_ > VARIABLE_max_[CONSTANT_self_]} :
;;	                          ?VARIABLE_num_#prime
;;	                          = [VARIABLE_num_ EXCEPT
;;	                               ![CONSTANT_self_] = CONSTANT_i_]
;;	                    /\ ?VARIABLE_pc_#prime
;;	                       = [VARIABLE_pc_ EXCEPT ![CONSTANT_self_] = "p4"] ,
;;	       VARIABLE_pc_[CONSTANT_self_] = "p3" ,
...
;;	PROVE  (/\ ?VARIABLE_num_#prime \in [1..CONSTANT_N_ -> Nat]
;;	        /\ ?VARIABLE_flag_#prime \in [1..CONSTANT_N_ -> BOOLEAN]
;;	        /\ ?VARIABLE_unread_#prime
;;	           \in [1..CONSTANT_N_ -> SUBSET 1..CONSTANT_N_]
;;	        /\ \A CONSTANT_i_ \in 1..CONSTANT_N_ :
;;	              ?VARIABLE_pc_#prime[CONSTANT_i_] \in {"p2", "p5", "p6"}
;;	              => CONSTANT_i_ \notin ?VARIABLE_unread_#prime[CONSTANT_i_]
;;	        /\ ?VARIABLE_max_#prime \in [1..CONSTANT_N_ -> Nat]
;;	        /\ ?VARIABLE_nxt_#prime \in [1..CONSTANT_N_ -> 1..CONSTANT_N_]
;;	        /\ \A CONSTANT_i_ \in 1..CONSTANT_N_ :
...
;;	                  {"p1", "p2", "p3", "p4", "p5", "p6", "cs", "p7"}])
;;	       /\ (\A CONSTANT_i_ \in 1..CONSTANT_N_ :
;;	              /\ /\ ?VARIABLE_pc_#prime[CONSTANT_i_] \in {"p1", "p2"}
...
;;	                    => ?VARIABLE_pc_#prime[CONSTANT_i_]
;;	                       \in {"p1", "p2", "p3", "p4"}
;;	                 /\ ?VARIABLE_pc_#prime[CONSTANT_i_] \in {"p2", "p3"}
;;	                    => ?VARIABLE_flag_#prime[CONSTANT_i_]
...
;;	                                 => CONSTANT_i_
;;	                                    \in ?VARIABLE_unread_#prime[CONSTANT_j_]
;;	                           \/ ?VARIABLE_pc_#prime[CONSTANT_j_] = "p7")
;;	              /\ (/\ ?VARIABLE_pc_#prime[CONSTANT_i_] = "p6"
...
;;	                    >= ?VARIABLE_num_#prime[CONSTANT_i_]
;;	              /\ ?VARIABLE_pc_#prime[CONSTANT_i_] = "cs"
;;	                 => (\A CONSTANT_j_ \in 1..CONSTANT_N_ \ {CONSTANT_i_} :
;;	                        /\ ?VARIABLE_num_#prime[CONSTANT_i_] > 0
;;	                        /\ \/ ?VARIABLE_pc_#prime[CONSTANT_j_] = "p1"
;;	                           \/ /\ ?VARIABLE_pc_#prime[CONSTANT_j_] = "p2"
...
;; Proof obligation #30
;; Generated from file "./examples/Bakery.tla", line 332, characters 5-6
...
(assert
  (!
    (distinct smt__TLAunderscore_underscore_StrLitunderscore_p1
      smt__TLAunderscore_underscore_StrLitunderscore_p2)
          ...
          (and (smt__TLAunderscore_underscore_Mem smt__x smt__a)
            (and
              (smt__TLAunderscore_underscore_IntLteq
                (smt__TLAunderscore_underscore_FunApp
                  smt__VARIABLEunderscore_maxunderscore_
                  smt__CONSTANTunderscore_selfunderscore_) smt__x)
              (not
                (smt__TLAunderscore_underscore_TrigEqunderscore_Idv
                  (smt__TLAunderscore_underscore_FunApp
                    smt__VARIABLEunderscore_maxunderscore_
...
(assert
  (exists ((smt__CONSTANTunderscore_repeatunderscore_ Idv))
    (and
      (smt__TLAunderscore_underscore_Mem
        smt__CONSTANTunderscore_repeatunderscore_
        smt__TLAunderscore_underscore_BoolSet)
      (exists ((smt__CONSTANTunderscore_kunderscore_ Idv))
        (and
          (smt__TLAunderscore_underscore_Mem
            smt__CONSTANTunderscore_kunderscore_
            smt__TLAunderscore_underscore_NatSet)
          (ite
            (= smt__CONSTANTunderscore_repeatunderscore_
              smt__TLAunderscore_underscore_Ttunderscore_Idv)
            (and
              (smt__TLAunderscore_underscore_TrigEqunderscore_Idv
                smt__VARIABLEunderscore_numunderscore_underscore_prime
                (smt__TLAunderscore_underscore_FunExcept
                  smt__VARIABLEunderscore_numunderscore_
                  smt__CONSTANTunderscore_selfunderscore_
                  smt__CONSTANTunderscore_kunderscore_))
              (smt__TLAunderscore_underscore_TrigEqunderscore_Idv
                smt__VARIABLEunderscore_pcunderscore_underscore_prime
                (smt__TLAunderscore_underscore_FunExcept
                  smt__VARIABLEunderscore_pcunderscore_
                  smt__CONSTANTunderscore_selfunderscore_
                  smt__TLAunderscore_underscore_StrLitunderscore_p3)))
            (and
              (exists ((smt__CONSTANTunderscore_iunderscore_ Idv))
                (and
                  (smt__TLAunderscore_underscore_Mem
                    smt__CONSTANTunderscore_iunderscore_
                    (smt__TLAunderscore_underscore_SetStunderscore_flatndunderscore_1
...
(assert
  (smt__TLAunderscore_underscore_TrigEqunderscore_Idv
    smt__VARIABLEunderscore_flagunderscore_underscore_prime
    smt__VARIABLEunderscore_flagunderscore_))

(assert
  (smt__TLAunderscore_underscore_TrigEqunderscore_Idv
    smt__VARIABLEunderscore_unreadunderscore_underscore_prime
    smt__VARIABLEunderscore_unreadunderscore_))

(assert
  (smt__TLAunderscore_underscore_TrigEqunderscore_Idv
    smt__VARIABLEunderscore_maxunderscore_underscore_prime
    smt__VARIABLEunderscore_maxunderscore_))

(assert
  (smt__TLAunderscore_underscore_TrigEqunderscore_Idv
    smt__VARIABLEunderscore_nxtunderscore_underscore_prime
    smt__VARIABLEunderscore_nxtunderscore_))

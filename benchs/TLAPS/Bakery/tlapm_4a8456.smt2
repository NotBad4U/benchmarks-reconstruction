...
;;	               ELSE /\ \E CST_i_
;;	                          \in {CST_j_ \in Nat :
;;	                                 CST_j_ > VAR_max_[CST_self_]} :
;;	                          ?VAR_num_#prime
;;	                          = [VAR_num_ EXCEPT
;;	                               ![CST_self_] = CST_i_]
;;	                    /\ ?VAR_pc_#prime
;;	                       = [VAR_pc_ EXCEPT ![CST_self_] = "p4"] ,
;;	       VAR_pc_[CST_self_] = "p3" ,
...
; omitted declaration of '_SetSt' (second-order)
...
;; Axiom: SetStDef _SetSt_flatnd_1
(assert
  (!
    (forall ((smt__a Idv) (smt__x Idv))
      (!
        ...
        :pattern ((Mem smt__x smt__a)
                   (SetSt_flatnd_1
                     smt__a)))) :named |SetStDef _SetSt_flatnd_1|))

(assert
  (exists ((CST_repeat_ Idv))
    (and
      (Mem
        CST_repeat_
        BoolSet)

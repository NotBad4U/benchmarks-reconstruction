...
;;	       NEW CONSTANT CST_self_ \in 1..CST_N_,
;;	       NEW CONSTANT CST_i_ \in 1..CST_N_,
;;	       \/ /\ ?VAR_flag_#prime
;;	             = [VAR_flag_ EXCEPT
;;	                  ![CST_self_] = ~VAR_flag_[CST_self_]]
;;	          /\ ?VAR_pc_#prime
;;	             = [VAR_pc_ EXCEPT ![CST_self_] = "p1"]
;;	       \/ /\ ?VAR_flag_#prime
;;	             = [VAR_flag_ EXCEPT ![CST_self_] = TRUE]
;;	          /\ ?VAR_pc_#prime
;;	             = [VAR_pc_ EXCEPT ![CST_self_] = "p2"] ,
;;	       VAR_pc_[CST_self_] = "p1" ,
;;	       ?VAR_unread_#prime
;;	       = [VAR_unread_ EXCEPT
;;	            ![CST_self_] = 1..CST_N_ \ {CST_self_}] ,
;;	       ?VAR_max_#prime = [VAR_max_ EXCEPT ![CST_self_] = 0] ,
;;	       ?VAR_num_#prime = VAR_num_ ,
;;	       ?VAR_nxt_#prime = VAR_nxt_ 
;;	PROVE  /\ /\ ?VAR_pc_#prime[CST_i_] \in {"p1", "p2"}
;;	             => ?VAR_num_#prime[CST_i_] = 0
;;	          /\ ?VAR_num_#prime[CST_i_] = 0
;;	             => ?VAR_pc_#prime[CST_i_] \in {"p1", "p2", "p3", "p7"}
;;	       /\ /\ ?VAR_flag_#prime[CST_i_]
;;	             => ?VAR_pc_#prime[CST_i_] \in {"p1", "p2", "p3", "p4"}
;;	          /\ ?VAR_pc_#prime[CST_i_] \in {"p2", "p3"}
;;	             => ?VAR_flag_#prime[CST_i_]
;;	       /\ ?VAR_pc_#prime[CST_i_] \in {"p5", "p6"}
;;	          => (\A CST_j_
;;	                 \in (1..CST_N_ \ ?VAR_unread_#prime[CST_i_])
;;	                     \ {CST_i_} :
;;	                 /\ ?VAR_num_#prime[CST_i_] > 0
